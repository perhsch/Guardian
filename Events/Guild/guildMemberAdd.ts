...
import { setLockdown } from '../../Functions/antiRaidLockdown.ts';
import { sendModLog } from '../../Functions/modLog.ts';
import { recordJoin, getJoinCount, getRecentJoiners } from '../../Functions/antiRaidTracking.ts';
...
export default {
    ...
    async execute(member: Discord.GuildMember, client: Discord.Client) {
        ...
        if (guild.logs.enabled) {
            ...
            let color: Discord.ColorResolvable = '#74e21e';
            let risk = 'Fairly Safe';

            const accountCreation = Math.floor(member.user.createdTimestamp / 1000);
            const joiningTime = Math.floor(member.joinedTimestamp! / 1000);
            ...
            const response: Discord.MessageCreateOptions = {
                embeds: [
