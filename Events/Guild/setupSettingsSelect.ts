...
function buildSetupSelectOptions(dbGuild: any): Discord.StringSelectMenuOptionBuilder[] {
    const needsLogging =
...
function parseIdInput(value: string | null): string | null {
    if (!value || typeof value !== 'string') return null;
    const trimmed = value.trim();
    return trimmed.length > 0 ? trimmed : null;
}

function buildLoggingModal(dbGuild: any): Discord.ModalBuilder {
    ...
    const optional = (id: string, label: string, value: string | null) => {
    ...
    return new Discord.ModalBuilder()
        ...
        .addComponents(
            new Discord.ActionRowBuilder<Discord.TextInputBuilder>().addComponents(basicInput),
...
function buildSuggestionModal(dbGuild: any): Discord.ModalBuilder {
    ...
    return new Discord.ModalBuilder()
        ...
        .addComponents(
            new Discord.ActionRowBuilder<Discord.TextInputBuilder>().addComponents(channelInput),
            new Discord.ActionRowBuilder<Discord.TextInputBuilder>().addComponents(reactionsInput)
        );
}

function buildTicketsModal(dbGuild: any): Discord.ModalBuilder {
    const makeRequired = (id: string, label: string, placeholder: string) => {
        const input = new Discord.TextInputBuilder()
            .setCustomId(id)
            .setLabel(label)
    ...
    async execute(interaction: Discord.Interaction, _client: Discord.Client) {
        ...
        if (interaction.isModalSubmit()) {
            ...
            if (customId === 'setup_modal_logging') {
                ...
                const validateChannel = async (id: string) => {
