...
const duplicateTracker = new Map<string, Map<string, number[]>>();

function containsZalgo(content: string): boolean {
...
function messageContainsInvite(message: Discord.Message): boolean {
...
function containsBadword(content: string, words: string[]): boolean {
...
function containsExcessiveCaps(content: string, threshold = 70, minLength = 10): boolean {
...
function isSpamming(userId: string, guildId: string, maxMessages = 5, timeframe = 5000): boolean {
...
async function executeAutomodAction(
    ...
    if (action === 'timeout') {
        ...
        if (member.moderatable) {
            ...
            await (client as any).expiringDocumentsManager.infractions.addNewDocument(inf);
        }
        const logEmbed = EmbedGenerator.basicEmbed(
...
export default {
    ...
    async execute(message: Discord.Message, client: Discord.Client) {
        ...
        const doc = dbGuild.document || ({} as any);
