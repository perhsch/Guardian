...
// @ts-ignore
import automodCommand from '../../Commands/Administrator/automod';

const TextInputStyle = Discord.TextInputStyle;

function buildAntiraidModal(dbGuild: any): Discord.ModalBuilder {
    const ar = dbGuild?.antiraid || {};
    const opt = (id: string, label: string, val: any, req = false) => {
        ...
        return new Discord.ActionRowBuilder<Discord.TextInputBuilder>().addComponents(input);
    };
...
function buildAntiZalgoModal(dbGuild: any): Discord.ModalBuilder {
    const az = dbGuild?.automod?.antiZalgo || {};
    const opt = (id: string, label: string, val: any) => {
        const input = new Discord.TextInputBuilder()
            .setCustomId(id)
            .setLabel(label)
            .setStyle(TextInputStyle.Short)
...
function buildAntiBadwordsModal(dbGuild: any): Discord.ModalBuilder {
    const ab = dbGuild?.automod?.antiBadwords || {};
    ...
    const opt = (id: string, label: string, val: any) => {
        ...
        return new Discord.ActionRowBuilder<Discord.TextInputBuilder>().addComponents(i);
    };
    ...
    return new Discord.ModalBuilder()
        ...
        .addComponents(
            ...
            new Discord.ActionRowBuilder<Discord.TextInputBuilder>().addComponents(input),
            opt('action', 'Action: delete, warn, or timeout', ab.action ?? 'delete')
        );
}

function buildAntiNukeModal(dbGuild: any): Discord.ModalBuilder {
    const an = dbGuild?.automod?.antiNuke || {};
    const opt = (id: string, label: string, val: any) => {
        const input = new Discord.TextInputBuilder()
            .setCustomId(id)
            .setLabel(label)
...
function buildAntiAdvertisementModal(dbGuild: any): Discord.ModalBuilder {
    const aa = dbGuild?.automod?.antiAdvertisement || {};
...
function buildAntiCapsModal(dbGuild: any): Discord.ModalBuilder {
    const ac = dbGuild?.automod?.antiCaps || {};
    const opt = (id: string, label: string, val: any) => {
        const input = new Discord.TextInputBuilder()
            .setCustomId(id)
            .setLabel(label)
...
function buildAntiMentionSpamModal(dbGuild: any): Discord.ModalBuilder {
    const ams = dbGuild?.automod?.antiMentionSpam || {};
    const opt = (id: string, label: string, val: any) => {
        const input = new Discord.TextInputBuilder()
            .setCustomId(id)
            .setLabel(label)
...
function buildAntiSpamModal(dbGuild: any): Discord.ModalBuilder {
    const as = dbGuild?.automod?.antiSpam || {};
    const opt = (id: string, label: string, val: any) => {
        const input = new Discord.TextInputBuilder()
            .setCustomId(id)
            .setLabel(label)
...
export default {
    ...
    async execute(interaction: Discord.Interaction, client: Discord.Client) {
        ...
        if (
            ...
            let modal: Discord.ModalBuilder | null = null;
        ...
        if (interaction.isModalSubmit() && interaction.customId.startsWith('automod_modal_')) {
            ...
            if (customId === 'automod_modal_antiraid') {
                ...
                dbGuild.antiraid.action = enabled ? action : null;
                dbGuild.antiraid.lockdown = dbGuild.antiraid.lockdown || {};
                dbGuild.antiraid.lockdown.enabled = lockdown;
                dbGuild.antiraid.channel = channel;
            ...
            if (customId === 'automod_modal_antizalgo') {
                ...
                if (!dbGuild.automod.antiZalgo) dbGuild.automod.antiZalgo = {};
                dbGuild.automod.antiZalgo.enabled = enabled;
                dbGuild.automod.antiZalgo.action = action;
            ...
            if (customId === 'automod_modal_antinuke') {
                ...
                if (!dbGuild.automod) dbGuild.automod = {};
                if (!dbGuild.automod.antiNuke) dbGuild.automod.antiNuke = {};
                dbGuild.automod.antiNuke.enabled = enabled;
                dbGuild.automod.antiNuke.maxChannelsPerMinute = maxChannels;
                dbGuild.automod.antiNuke.maxRolesPerMinute = maxRoles;
                dbGuild.automod.antiNuke.action = action;
            ...
            if (customId === 'automod_modal_anticaps') {
                ...
                if (!dbGuild.automod) dbGuild.automod = {};
                if (!dbGuild.automod.antiCaps) dbGuild.automod.antiCaps = {};
                dbGuild.automod.antiCaps.enabled = enabled;
                dbGuild.automod.antiCaps.threshold = Math.max(1, Math.min(100, threshold));
                dbGuild.automod.antiCaps.minLength = Math.max(1, minLength);
                dbGuild.automod.antiCaps.action = action;
            ...
            if (customId === 'automod_modal_antimentionspam') {
                ...
                dbGuild.automod.antiMentionSpam.action = action;
            ...
            if (customId === 'automod_modal_antispam') {
                ...
                if (!dbGuild.automod) dbGuild.automod = {};
                if (!dbGuild.automod.antiSpam) dbGuild.automod.antiSpam = {};
                dbGuild.automod.antiSpam.enabled = enabled;
                dbGuild.automod.antiSpam.maxMessages = Math.max(1, maxMessages);
                dbGuild.automod.antiSpam.timeframe = Math.max(1000, timeframe);
                dbGuild.automod.antiSpam.maxDuplicates = Math.max(1, maxDuplicates);
                dbGuild.automod.antiSpam.action = action;
