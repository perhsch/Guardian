...
export default {
    ...
    data: new SlashCommandSubcommandBuilder()
        ...
        .addStringOption((option) =>
            option.setName('user_id').setDescription('The Discord user ID of the user to add a note for.').setRequired(true)
        )
        .addStringOption((option) =>
    ...
    async execute(interaction: ChatInputCommandInteraction, client: Client, dbGuild: any) {
        ...
        const note = await Notes.create({ guild: interaction.guild.id, user: userId, content, author: interaction.user.id });
        ...
        return { embeds: [EmbedGenerator.basicEmbed(`Note added. Use note ID \`${note.noteId}\` to remove this note later.`)] };
    },
};
