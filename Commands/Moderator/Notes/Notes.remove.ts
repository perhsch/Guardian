...
export default {
    ...
    data: new SlashCommandSubcommandBuilder()
        ...
        .addStringOption((option) =>
            option.setName('user_id').setDescription('The Discord user ID of the user the note belongs to.').setRequired(true)
        )
        .addStringOption((option) =>
            option.setName('note_id').setDescription('The unique note ID (shown when viewing or adding notes).').setRequired(true)
        ),
    ...
    async execute(interaction: ChatInputCommandInteraction, client: Client, dbGuild: any) {
        ...
        const note = await Notes.findOneAndDelete({ guild: interaction.guild.id, user: userId, noteId });
