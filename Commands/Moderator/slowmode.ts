import { SlashCommandBuilder, PermissionFlagsBits, ChatInputCommandInteraction, Client, EmbedBuilder, TextChannel } from 'discord.js';
import ms from 'ms';
...
export default {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((option) =>
            option.setName('duration').setDescription('Duration between sending messages in the channel.').setRequired(true)
        )
        .addStringOption((option) =>
    ...
    async execute(interaction: ChatInputCommandInteraction, client: Client, dbGuild: any) {
        ...
        channel.setRateLimitPerUser(durationSeconds, reason)
            .then(async () => {
                const durationString = durationSeconds === 0 ? 'disabled' : ms(durationMs!, { long: true });

                const successEmbed = new EmbedBuilder()
                    .setColor(durationSeconds === 0 ? 0x57f287 : 0xfee75c)
                    .setTitle(`${durationSeconds === 0 ? '🟢' : '⏱️'} Slowmode Updated`)
                    .setThumbnail(interaction.guild!.iconURL({ size: 256 }))
                    .setDescription(`>>> **Channel slowmode has been successfully updated!**\n\n📝 **Details**\n• **Channel**: ${channel.toString()}\n• **Duration**: \`${durationString}\`\n• **Reason**: ${reason}`)
                    .addFields(
                        { name: '🔧 Configuration', value: `• **Set by**: ${interaction.user.toString()}\n• **Time**: <t:${Math.floor(Date.now() / 1000)}:R>`, inline: true },
                        { name: '📊 Status', value: `• **Previous**: \`${channel.rateLimitPerUser}s\`\n• **New**: \`${durationSeconds}s\``, inline: true }
                    )
                    .setFooter({ text: `Guardian Moderation • ${interaction.guild!.name}`, iconURL: client.user!.displayAvatarURL() })
                    .setTimestamp();

                const logEmbed = new EmbedBuilder()
                    .setColor(0xfee75c)
                    .setTitle('⚙️ Slowmode Configuration')
                    .setDescription(`>>> **Channel slowmode settings have been modified**\n\n**Action Details**\n• **Moderator**: ${interaction.user.toString()}\n• **Channel**: ${channel.toString()}\n• **New Duration**: \`${durationString}\`\n• **Reason**: ${reason}`)
                    .setTimestamp();
