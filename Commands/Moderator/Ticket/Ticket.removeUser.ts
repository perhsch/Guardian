...
export default {
    ...
    data: new SlashCommandSubcommandBuilder()
        ...
        .addChannelOption((option) => option.setName('channel').setDescription('Ticket to remove the user from.')),
    ...
    async execute(interaction: ChatInputCommandInteraction, _client: Client, dbGuild: any) {
        ...
        if (ticket.user === user.id) return { embeds: [EmbedGenerator.errorEmbed('Unable to remove the ticket creator.')] };
