import { SlashCommandSubcommandBuilder, ChatInputCommandInteraction, Client, GuildMember, ChannelType, TextChannel } from 'discord.js';
...
export default {
    ...
    data: new SlashCommandSubcommandBuilder()
        ...
        .addChannelOption((option) =>
            option.setName('channel').setDescription('Ticket to close').addChannelTypes(ChannelType.GuildText)
        ),
    ...
    async execute(interaction: ChatInputCommandInteraction, client: Client, dbGuild: any) {
        ...
        if (!isInTicketChannel && !hasTicketStaff(interaction, dbGuild))
            return { embeds: [EmbedGenerator.errorEmbed('You need the ticket staff role to close tickets from another channel.')] };
        if (isInTicketChannel && !isCreator && !hasTicketStaff(interaction, dbGuild))
            return { embeds: [EmbedGenerator.errorEmbed('Only ticket staff or the ticket creator can close this ticket.')] };

        await interaction.deferReply();
        await closeTicketChannel(client, interaction.guild, channel, ticket, interaction.user.id, dbGuild?.tickets?.logChannel);
