...
export default {
    ...
    async execute(interaction: ChatInputCommandInteraction, client: Client, dbGuild: any, dbUser: any) {
        ...
        const tickets = await Tickets.find({ guild: interaction.guild.id, user: user.id, active: false });
        if (tickets.length === 0) return { embeds: [EmbedGenerator.errorEmbed('No previous tickets found.')] };

        const embeds = [];
        for (let i = 0; i < tickets.length; i += 10) {
            const ticketsSlice = tickets.slice(i, i + 10);
            const baseUrl = process.env.LIVE === 'true' ? 'https://guardianbot.space' : 'http://localhost:3001';
            const embed = EmbedGenerator.basicEmbed(
                ticketsSlice.map((ticket: any) => `[${ticket._id.toString()}](${baseUrl}/guilds/${ticket.guild}/tickets/${ticket._id.toString()})`).join('\n')
            ).setTitle('Previous Tickets').setTimestamp();
            embeds.push(embed);
        }
