...
export default {
    ...
    async execute(interaction: ChatInputCommandInteraction, _client: Client, dbGuild: any) {
        ...
        if (!(await interaction.guild.bans.fetch(user.id).catch(() => null))) {
            return { embeds: [EmbedGenerator.errorEmbed('That user is not banned')], ephemeral: true };
        }

        interaction.guild.members.unban(user, reason)
            .then(async () => {
                await Infractions.updateMany({ type: 'ban', user: user.id, guild: interaction.guild!.id }, { $set: { active: false } });

                const logEmbed = EmbedGenerator.basicEmbed(
                    [`- Moderator: ${interaction.user.tag}`, `- Target: ${user.tag} (${user.id})`, `- Reason: ${reason}`].join('\n')
