...
interface SetupStep {
    id: number;
    label: string;
    warningOnFailure: boolean;
    check: (dbGuild: any) => boolean;
}
...
export default {
    ...
    async execute(interaction: ChatInputCommandInteraction, client: Client, dbGuild: any) {
        ...
        if (!interaction.guild.members.me?.permissions.has(PermissionFlagsBits.ModerateMembers)) {
            ...
            embed
                ...
                .setDescription(
                    [
                        '```diff',
                        '- Bot is missing the Moderate Members permissions! -',
                        '```',
                        '',
                        "❌ Please grant the `Administrator` or `Moderate Members` permission to the bot's role and run `/setup` again.",
                    ].join('\n')
                );
        ...
        if (dbGuild.document) dbGuild.document.setup = true;
    },
};

function generateEmbed(count: number, completed: number[], dbGuild: any) {
    const stepInfo = [
