...
export default {
    ...
    async execute(interaction: ChatInputCommandInteraction, client: Client, dbGuild: any) {
        ...
        if (!role) {
            ...
            if (!role) {
                return interaction.reply({ embeds: [EmbedGenerator.errorEmbed('Failed to create role.')], ephemeral: true });
            }
        }
        ...
        if (!category) {
            category = await interaction.guild.channels.create({
                ...
                permissionOverwrites: [
                    { id: interaction.guild.roles.everyone, deny: ['ViewChannel', 'SendMessages'] },
                    { id: role.id, allow: ['ViewChannel', 'SendMessages'] },
                ],
            }).catch(() => null) as CategoryChannel | null;
            if (!category) {
                return interaction.reply({ embeds: [EmbedGenerator.errorEmbed('Failed to create category.')], ephemeral: true });
            }
        }
        ...
        if (!staffChannel) {
            staffChannel = await interaction.guild.channels.create({
                ...
                parent: category.id,
                permissionOverwrites: [
                    { id: interaction.guild.roles.everyone.id, deny: ['ViewChannel', 'SendMessages', 'ReadMessageHistory'] },
                    { id: role.id, allow: ['ViewChannel', 'SendMessages', 'ReadMessageHistory'] },
                ],
            }).catch(() => null) as TextChannel | null;
            if (!staffChannel) {
                return interaction.reply({ embeds: [EmbedGenerator.errorEmbed('Failed to create staff channel.')], ephemeral: true });
            }
        } else {
            await staffChannel.setParent(category.id).catch(() => null);
            await staffChannel.permissionOverwrites.edit(interaction.guild.roles.everyone.id, { ViewChannel: false, SendMessages: false, ReadMessageHistory: false }).catch(() => null);
            await staffChannel.permissionOverwrites.edit(role.id, { ViewChannel: true, SendMessages: true, ReadMessageHistory: true }).catch(() => null);
        }
        ...
        if (!channel) {
            ...
            if (!channel) {
                return interaction.reply({ embeds: [EmbedGenerator.errorEmbed('Failed to create ticket channel.')], ephemeral: true });
            }
        } else {
